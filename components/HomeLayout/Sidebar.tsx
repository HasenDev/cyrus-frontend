...
function SidebarContent({ isOpen, setIsOpen }: SidebarProps) {
  ...
  const [isUnder240, setIsUnder240] = useState(false);

  useEffect(() => {
    const handleResize = () => {
      setIsUnder240(window.innerWidth < 240);
    };
  ...
  const isMenuItemActive = (href: string) => {
    ...
    if (href === "/home/admin/payment") {
      return pathname.startsWith("/home/admin/payment") || pathname.startsWith("/home/admin/payments");
    }
  ...
  const menuItems = [
    ...
    {
      name: isUnder240 ? "Shop" : "Shop Packages",
      href: "/home/packages",
      icon: (
        <svg className="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
  ...
  const adminMenuItems = [
    ...
    },
    {
      ...
      perm: "ADMIN_SETTINGS",
      icon: <ScaleIcon className="h-5 w-5 shrink-0" />,
    },
    {
      name: "Payment",
    ...
    {
      name: isUnder240 ? "News" : "Announcements",
      href: "/home/admin/announcements",
      perm: "ADMIN_ANNOUNCEMENTS",
      icon: <MegaphoneIcon className="h-5 w-5 shrink-0" />,
    },
    {
      name: "Locations",
  ...
  const formattedBalance = parseCredits(user?.metrics?.credits);
  ...
  return (
    <>
      ...
      <aside
        className={`fixed bottom-0 top-0 left-0 z-50 flex w-64 max-w-[calc(100vw-1.5rem)] flex-col overflow-hidden border-r rounded-br-2xl rounded-tr-2xl md:rounded-tr-none transition-transform duration-300 ease-in-out md:translate-x-0 ${
          isDark
      ...
      >
        <div
          className={`flex min-h-16 w-full max-w-full overflow-hidden ${
            isUnder240
              ? "flex-col items-start gap-2"
              : "flex-wrap items-center justify-between gap-2 max-[239px]:flex-col max-[239px]:items-start"
          } px-4 sm:px-6 py-3.5 border-b ${
            isDark ? "border-white/[0.06]" : "border-zinc-200"
          }`}
        >
          <div className="flex items-center gap-2.5 sm:gap-3 min-w-0 w-full max-w-full flex-1 overflow-hidden">
            <img
              src={panelIconSrc}
              onError={() => setPanelIconSrc("/icon.png")}
              alt="Logo"
              className="h-7 w-7 sm:h-8 sm:w-8 object-contain shrink-0 rounded"
            />
            <span
              className="text-base sm:text-lg font-black tracking-wider truncate uppercase min-w-0 flex-1 block"
          ...
          <button
            onClick={() => setIsOpen(false)}
            className={`md:hidden p-1 transition-colors shrink-0 ${
              isUnder240
                ? "self-start ml-0"
                : "ml-auto max-[239px]:ml-0 max-[239px]:self-start"
            } ${
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden no-scrollbar">
          <nav className="space-y-1 px-4 py-6">
            <p className={`px-4 mb-2 text-[10px] font-bold uppercase tracking-widest ${isDark ? "text-zinc-500" : "text-zinc-400"}`}>
              General
            </p>
            {menuItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  onClick={() => setIsOpen(false)}
                  className="flex items-center gap-3 rounded-lg px-4 py-2.5 text-sm font-semibold transition-all duration-200 min-w-0 overflow-hidden"
                  style={{
                    color: isActive
                      ? isDark
                        ? "#ffffff"
          ...
          {adminMenuItems.length > 0 && (
            <nav className={`space-y-1 px-4 py-4 border-t ${isDark ? "border-white/[0.04]" : "border-zinc-200/60"}`}>
              <p className={`px-4 mb-2 text-[10px] font-bold uppercase tracking-widest ${isDark ? "text-zinc-500" : "text-zinc-400"}`}>
                Administration
              </p>
            ...
            <nav className={`space-y-1 px-4 py-4 border-t ${isDark ? "border-white/[0.04]" : "border-zinc-200/60"}`}>
              ...
              {websiteUrl && (
                <a
                  ...
                  onClick={() => setIsOpen(false)}
                  className={`group flex items-center justify-between gap-3 rounded-lg px-4 py-2.5 text-sm font-semibold transition-all duration-200 min-w-0 overflow-hidden ${
                    isDark
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <FaGlobe className="h-5 w-5 shrink-0" />
        ...
        {user && (
          <div
            className={`border-t px-4 py-3.5 transition-colors overflow-hidden ${
              isDark ? "border-white/[0.06]" : "border-zinc-200"
            }`}
          >
            <div className="flex items-center gap-3 min-w-0">
