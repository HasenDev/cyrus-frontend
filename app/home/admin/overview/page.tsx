...
function compareVersions(a: string, b: string): number {
  const normalize = (version: string) =>
    version.replace(/^v/i, "").split(".").map((num) => parseInt(num, 10) || 0);
...
export default function AdminOverviewPage() {
  ...
  useEffect(() => {
    async function fetchAll() {
      ...
      try {
        const [overviewRes, releaseRes] = await Promise.allSettled([
          ...
          fetch("https://api.github.com/repos/HasenDev/cyrus-panel/releases/latest", {
  ...
  const skeletonBg = isDark ? "bg-white/[0.05]" : "bg-zinc-200/80";

  const stats = [
    {
      name: "Panel Version",
      value: formattedCurrentVersion,
      icon: (
        <svg
          className="h-6 w-6"
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="grid gap-6 sm:grid-cols-3">
        {loading
          ? Array.from({ length: 3 }).map((_, idx) => (
              <div
                key={idx}
                className={`rounded-xl border p-6 shadow-sm animate-pulse space-y-4 ${
                  isDark
              ...
              >
                <div className="flex items-center justify-between">
                  <div className={`h-3 w-24 rounded-md ${skeletonBg}`} />
                  <div className={`h-6 w-6 rounded-md ${skeletonBg}`} />
                </div>
                <div className={`h-8 w-28 rounded-lg ${skeletonBg}`} />
              </div>
            ))
          : stats.map((stat) => (
              <div
      ...
      <div
        className={`rounded-xl border p-6 sm:p-8 shadow-sm transition-colors ${
          isDark
            ? "border-white/[0.06] bg-[#0F1014]"
            : "border-zinc-200 bg-white"
        }`}
        ...
        {loading ? (
          <div className="space-y-6 animate-pulse">
            <div className="space-y-3">
              <div className={`h-3 w-28 rounded-md ${skeletonBg}`} />
              <div className={`h-6 w-3/4 max-w-lg rounded-lg ${skeletonBg}`} />
              <div className={`h-3.5 w-1/2 max-w-sm rounded-md ${skeletonBg}`} />
            </div>
            <div className="flex flex-wrap gap-3 pt-2">
              <div className={`h-9 w-40 rounded-lg ${skeletonBg}`} />
              <div className={`h-9 w-32 rounded-lg ${skeletonBg}`} />
              <div className={`h-9 w-32 rounded-lg ${skeletonBg}`} />
            </div>
          </div>
        ) : (
                ...
                {isUpToDate ? (
                  <>
                    ...
                    <span style={{ color: accentColor }}>
                      Cyrus Panel {formattedCurrentVersion}
                    </span>
            ...
            <div className="flex flex-wrap gap-3 pt-2">
              {!isUpToDate && latestRelease && (
                <a
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg text-xs font-bold text-black hover:opacity-90 transition-all shadow-md"
                >
                  <svg
                    className="h-4 w-4"
                    fill="none"
                  ...
                  View Release ({formattedLatestTag})
                </a>
              )}
